...
function CheckoutSuccess() {
  ...
  const { data, isLoading, isError } = useVerifyCheckout(
    { sessionId },
    { query: { enabled: Boolean(sessionId), queryKey: getVerifyCheckoutQueryKey({ sessionId }) } },
  );

  useEffect(() => {
    if (data?.verified && data.reportToken) {
      setLocation(`/report/${data.reportToken}`, { replace: true });
    }
  }, [data, setLocation]);

  return <Layout><main className="not-found">
    <div className="eyebrow">Payment confirmation</div>
    {isLoading && <><h1 className="display">Confirming your report.</h1><p className="body-copy">Stripe is confirming the payment securely. This usually takes a few seconds.</p><Loading /></>}
    {isError && <><h1 className="display">We could not confirm that payment.</h1><p className="body-copy">Your payment may still be processing. Please check your email or contact us before trying again.</p><Link href="/contact" className="button button-primary">Contact support <ArrowRight size={15} /></Link></>}
    {!isLoading && !isError && data && !data.verified && <><h1 className="display">Payment is still processing.</h1><p className="body-copy">Stripe reports a status of {data.status}. Refresh this page in a moment, or contact us if it does not resolve.</p><button className="button button-primary" onClick={() => window.location.reload()}>Check again <ArrowRight size={15} /></button></>}
    {!sessionId && <><h1 className="display">No payment session found.</h1><p className="body-copy">Return to your preview to begin again.</p><Link href="/preview" className="button button-primary">Return to preview <ArrowRight size={15} /></Link></>}
  </main></Layout>;
}
...
function Router() {
  return <Switch>
    <Route path="/" component={Home} /><Route path="/create" component={Create} /><Route path="/preview" component={Preview} /><Route path="/how-it-works" component={HowItWorks} /><Route path="/example" component={Example} /><Route path="/pricing" component={Pricing} /><Route path="/faq" component={FAQ} /><Route path="/about" component={About} /><Route path="/contact" component={Contact} /><Route path="/privacy"><Legal kind="privacy" /></Route><Route path="/terms"><Legal kind="terms" /></Route><Route path="/report/:token" component={Report} /><Route path="/checkout/success" component={CheckoutSuccess} /><Route path="/admin" component={Admin} /><Route component={NotFound} />
  </Switch>;
}
